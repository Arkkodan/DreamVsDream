...
void HitBox::draw(int _x, int _y, bool attack, bool selected)
{
    ...
    if(attack) glColor4f(1.0f, 0.0f, 0.0f, 1.0f);
    else glColor4f(0.5f, 0.5f, 1.0f, 1.0f);
    ...
    if(selected)
    {
        if(attack) glColor4f(1.0f, 0.0f, 0.0f, 0.5f);
...
bool HitBox::collideOther(HitBox* other, Vector* colpos)
{
    if(pos.x + size.x < other->pos.x) return false;
    if(pos.x > other->pos.x + other->size.x) return false;
    if(pos.y + size.y < other->pos.y) return false;
    if(pos.y > other->pos.y + other->size.y) return false;
    ...
    if(colpos)
    {
        ...
        if(pos.x < other->pos.x) colpos->x = other->pos.x + (pos.x + size.x - other->pos.x) / 2;
        else colpos->x = pos.x + (other->pos.x + other->size.x - pos.x) / 2;
        if(pos.y < other->pos.y) colpos->y = other->pos.y + (pos.y + size.y - other->pos.y) / 2;
        else colpos->y = pos.y + (other->pos.y + other->size.y - pos.y) / 2;
    }
    return true;
}
...
HitBox HitBox::adjust(int _x, int _y, bool m, float scale)
{
    ...
    if(m) adj.pos.x = _x + pos.x * -scale + size.x * -scale;
    else adj.pos.x = _x + pos.x * scale;
    adj.pos.y = _y + pos.y * scale;

    return adj;
}

int Sprite::collide(int x1, int y1, int x2, int y2, bool m1, bool m2, float scale1, float scale2, Sprite* other, Vector* colpos)
{
...
void Sprite::drawShadow(int _x, bool mirror, float scale)
{
    ...
    //if(Graphics::shader_support) Graphics::setShader(SHADER_SHADOW);
    //else Graphics::setColor(0, 0, 0, 0.5f);
...
void Spark::think()
{
    ...
    if(++timer >= effects[group].effects[effect].speeds[frame])
    {
        ...
        {
            if(frame < effects[group].effects[effect].frameC - 1) frame++;
            else
            {
                alpha -= 0.25;
                if(alpha <= 0.0)
...
void newSpark(int x, int y, int vx, int vy, int group, bool mirror)
{
    ...
    sparks[sparkCounter].effect = roll(effects[group].size);
    ...
    if(++sparkCounter >= SPARK_COUNT) sparkCounter = 0;
}
...
void newSpark(int x, int y, int group, bool mirror)
{
    ...
    if(effects[group].type == EFFECT_SCATTER)
    {
        int _mirror = 1;
        if(mirror) _mirror = -1;
        int particles = roll(1, 5);
        for(int i = 0; i < particles; i++)
            newSpark(x, y, roll(3, 13) * _mirror, roll(5, 15), 1, mirror);
    }
}
