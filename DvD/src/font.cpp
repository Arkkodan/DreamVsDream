...
void Font::drawText(int x, int y, std::string text, ubyte_t r, ubyte_t g, ubyte_t b, float a)
{
    ...
    for(int i = 0; text[i]; i++)
    {
        ...
        else
        {
            ...
            img.draw(x, y);
            if(mono) x += mono * xscale;
            else x += (width[(ubyte_t)c] + 1) * xscale;
        }
    }
...
int Font::getTextWidth(std::string text)
{
    ...
    {
        int w = 0;
        for(const char* sz = text.c_str(); *sz; sz++) w += getCharWidth(*sz);
        return w;
    }
