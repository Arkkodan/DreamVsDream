...
void MenuIntro::draw()
{
    ...
    if(state % 2 == 0) _alpha = 1.0 - _alpha;
...
void MenuTitle::init()
{
    ...
    if(nThemes)
    {
        //Roll a weighted random number, favoring the "normal" title
        int i = roll(nThemes + 5);
        i -= 5;
        if(i < 0) i = 0;
...
void MenuTitle::think()
{
    ...
    if(input(up))
    {
        ...
        choiceLast = choice;
        if(choice == 0) choice = menuChoicesMax[submenu] - 1;
        else choice--;
    }
    else if(input(down))
    {
        ...
        choiceTimer = aXOffset;
        choiceLast = choice;
        if(choice >= menuChoicesMax[submenu] - 1) choice = 0;
        else choice++;
    }
...
void MenuSelect::think()
{
    ...
    if(state < 2)
    {
        for(int cur = 0; cur < 2; cur++)
        {
            ...
            if(cur == 1) input = poniko.frameInput;
            ...
            if(!cursors[cur].locked)
            {
                ...
                if(input & INPUT_LEFT)
                {
                    ...
                    do
                    {
                        if(cursors[cur].pos % width == 0) cursors[cur].pos += width - 1;
                        else cursors[cur].pos --;
                    }
                ...
                else if(input & INPUT_RIGHT)
                {
                    ...
                    do
                    {
                        if(cursors[cur].pos % width == width - 1) cursors[cur].pos -= width - 1;
                        else cursors[cur].pos++;
                    }
                    while(grid[cursors[cur].pos].x == -1);
                ...
                else if(input & INPUT_UP)
                {
                    ...
                    do
                    {
                        if(cursors[cur].pos / width == 0) cursors[cur].pos += width * (height - 1);
                        else cursors[cur].pos -= width;
                    }
                    while(grid[cursors[cur].pos].x == -1);
                ...
                else if(input & INPUT_DOWN)
                {
                    ...
                    do
                    {
                        if(cursors[cur].pos / width == height - 1) cursors[cur].pos -= width * (height - 1);
                        else cursors[cur].pos += width;
                    }
                    while(grid[cursors[cur].pos].x == -1);
    ...
    else
    {
        ...
        {
            //Start game!
            madotsuki.fighter = &fighters[gridFighters[cursors[0].pos]];
            poniko.fighter = &fighters[gridFighters[cursors[1].pos]];
...
void MenuSelect::draw()
{
    ...
    if(state >= 1 || FIGHT->gametype == GAMETYPE_VERSUS)
    {
        if(cursors[1].timerPortrait)
        {
            if(gridFighters[cursors[1].posOld] >= 0)
            {
                ...
                fighters[gridFighters[cursors[1].posOld]].portrait.draw(WINDOW_WIDTH - fighters[gridFighters[cursors[1].posOld]].portrait.w + (PORTRAIT_FADE - cursors[1].timerPortrait), 0, true);
            }
        }
        if(gridFighters[cursors[1].pos] >= 0)
        {
            Graphics::setColor(255, 255, 255, (float)(PORTRAIT_FADE - cursors[1].timerPortrait) / PORTRAIT_FADE);
            fighters[gridFighters[cursors[1].pos]].portrait.draw(WINDOW_WIDTH - fighters[gridFighters[cursors[1].pos]].portrait.w + cursors[1].timerPortrait, 0, true);
        }
    }
    if(cursors[0].timerPortrait)
    {
        if(gridFighters[cursors[0].posOld] >= 0)
        {
            ...
            fighters[gridFighters[cursors[0].posOld]].portrait.draw(0 - (PORTRAIT_FADE - cursors[0].timerPortrait), 0);
        }
    }
    if(gridFighters[cursors[0].pos] >= 0)
    {
        Graphics::setColor(255, 255, 255, (float)(PORTRAIT_FADE - cursors[0].timerPortrait) / PORTRAIT_FADE);
        fighters[gridFighters[cursors[0].pos]].portrait.draw(0 - cursors[0].timerPortrait, 0);
    }

    //Now the GUI
    if(gui) gui->draw(false);
    ...
    if(state >= 2)
    {
        ...
        {
            if(cursor_stage == i)
                font_stage.drawText(32+16, 32 + 32 * i, sz_stages[i], 180, 120, 190);
            else
                font_stage.drawText(32+16, 32 + 32 * i, sz_stages[i]);
        }
...
void MenuOptions::think()
{
    ...
    else
    {
        ...
        if(input(INPUT_UP))
        {
            sndMenu.play();
            if(cursor) cursor--;
            else cursor = OPTION_MAX - 1;
        }
        else if(input(INPUT_DOWN))
        {
            sndMenu.play();
            if(cursor < OPTION_MAX - 1) cursor++;
            else cursor = 0;
        }
        if(stopped)
        {
        ...
        if(input(INPUT_LEFT) || input(INPUT_RIGHT))
        {
            ...
            case OPTION_DIFFICULTY:
                if(input(INPUT_LEFT))
                {
                    if(optionDifficulty > 1) optionDifficulty--;
                }
                else
                {
                    if(optionDifficulty < 5) optionDifficulty++;
                }
                break;
            ...
            case OPTION_WINS:
                if(input(INPUT_LEFT))
                {
                    if(optionWins > 1) optionWins--;
                }
                else
                {
                    if(optionWins < 3) optionWins++;
                }
                break;
            ...
            case OPTION_TIME:
                if(input(INPUT_LEFT))
                {
                    if(optionTime == 0) optionTime = 99;
                    else if(optionTime == 99) optionTime = 60;
                }
                else
                {
                    if(optionTime == 60) optionTime = 99;
                    else if(optionTime == 99) optionTime = 0;
                }
                break;
            ...
            case OPTION_SFX_VOLUME:
                if(input(INPUT_LEFT))
                {
                    if(optionSfxVolume > 0) optionSfxVolume -= 10;
                }
                else
                {
                    if(optionSfxVolume < 100) optionSfxVolume += 10;
                }
                break;
            ...
            case OPTION_MUS_VOLUME:
                if(input(INPUT_LEFT))
                {
                    if(optionMusVolume > 0) optionMusVolume -= 10;
                }
                else
                {
                    if(optionMusVolume < 100) optionMusVolume += 10;
                }
#ifdef EMSCRIPTEN
		Mix_VolumeMusic(optionMusVolume * MIX_MAX_VOLUME / 100);
            ...
            case OPTION_VOICE_VOLUME:
                if(input(INPUT_LEFT))
                {
                    if(optionVoiceVolume > 0) optionVoiceVolume -= 10;
                    madotsuki.speaker.play(&dame);
                }
                else
                {
                    if(optionVoiceVolume < 100) optionVoiceVolume += 10;
...
void MenuOptions::init()
{
    ...
    if(nThemes)
    {
        parseFile(getResource(themes[roll(nThemes)], EXT_SCRIPT));
    }
}
...
void MenuMeter::draw(float pct, bool mirror, bool flip)
{
    if(pct > 0)
    {
        if(flip) Graphics::setRect(0, 0, img.w * pct, img.h);
        else Graphics::setRect(img.w * (1-pct), 0, img.w * pct + 1, img.h);

        if(mirror)
        {
            if(flip) img.draw(WINDOW_WIDTH - img.w - pos.x + img.w * (1 - pct), pos.y, true);
            else
            {
                //Hack
                if(pct == 1.0f)
        ...
        {
            if(flip) img.draw(pos.x, pos.y);
            else img.draw(pos.x + img.w * (1 - pct), pos.y);
        }
    }
}
    ...
    else
    {
        ...
        if(game_timer && !timer_ko && !timer_round_in && !timer_round_out)
        {
            if(!--game_timer)
            {
                ...
                {
                    if(madotsuki.inStandardState(STATE_DASH_FORWARD))
                       madotsuki.setStandardState(STATE_DASH_FORWARD_END);
                    if(madotsuki.inStandardState(STATE_DASH_BACK))
                       madotsuki.setStandardState(STATE_DASH_BACK_END);
                }
                if(poniko.flags & F_ON_GROUND && poniko.isDashing())
                {
                    if(poniko.inStandardState(STATE_DASH_FORWARD))
                       poniko.setStandardState(STATE_DASH_FORWARD_END);
                    if(poniko.inStandardState(STATE_DASH_BACK))
                       poniko.setStandardState(STATE_DASH_BACK_END);
                }
            }
        }
        ...
        {
            comboLeftTimer--;
            if(comboLeftOff < comboLeft.w) comboLeftOff += 16;
            if(comboLeftOff > comboLeft.w) comboLeftOff = comboLeft.w;
        }
        else if(comboLeftOff && (madotsuki.comboCounter < 2 || ko_player))
        {
            if(comboLeftOff > 0) comboLeftOff -= 16;
            if(comboLeftOff <= 0)
            {
                comboLeftOff = 0;
                comboLeftLast = 0;
        ...
        {
            comboRightTimer--;
            if(comboRightOff < comboRight.w) comboRightOff += 16;
            if(comboRightOff > comboRight.w) comboRightOff = comboRight.w;
        }
        else if(comboRightOff && (poniko.comboCounter < 2 || ko_player))
        {
            if(comboRightOff > 0) comboRightOff -= 16;
            if(comboRightOff <= 0)
            {
                comboRightOff = 0;
                comboRightLast = 0;
        ...
        if(timer_round_out == (int)(1.5 * FPS)) fadeoutSnd.play();
        ...
        if(timer_round_in) timer_round_in--;
        if(timer_round_out)
        {
            timer_round_out--;
            if(timer_round_out == (int)(3.8 * FPS))
            {
                if(ko_player == 2)
                {
                    if(!(poniko.flags & F_DEAD)) poniko.setStandardState(STATE_DEFEAT);
                    madotsuki.setStandardState(STATE_VICTORY);
                    win_types[0][wins[0]++] = 0;
                }
                else if(ko_player == 1)
                {
                    if(!(madotsuki.flags & F_DEAD)) madotsuki.setStandardState(STATE_DEFEAT);
                    poniko.setStandardState(STATE_VICTORY);
        ...
        if(ko_player && !timer_ko && !timer_round_out)
        {
            ...
            if(ko_type == 0)
            {
                if(ko_player == 2)
                {
                    _condition = madotsuki.isIdle() && (madotsuki.flags & F_ON_GROUND) &&
                                (poniko.inStandardState(STATE_PRONE) || poniko.inStandardState(STATE_ON_BACK));
                }
                else if(ko_player == 1)
                {
                    _condition = poniko.isIdle() && (poniko.flags & F_ON_GROUND) &&
                                (madotsuki.inStandardState(STATE_PRONE) || madotsuki.inStandardState(STATE_ON_BACK));
                }
            }
            else if(ko_type == 1)
            {
            ...
            if(_condition) timer_round_out = 4 * FPS;
        }
    }
}
...
void MenuFight::draw()
{
    ...
    else
    {
        ...
        if(timer_flash)
        {
            ...
            staticImg.draw(-roll(WINDOW_WIDTH), -roll(WINDOW_HEIGHT));
            timer_flash--;
        }
        ...
        if(timer_round_in)
        {
            ...
            if(timer_round_in <= 1.4 * FPS)
            {
                if(timer_round_in > 1.3 * FPS)
                {
                    ...
                    round_splash[round].draw(WINDOW_WIDTH / 2 - round_splash[round].w * scalar / 2 - roll(10, 30), WINDOW_HEIGHT / 2 - round_splash[round].h * scalar / 2 - roll(10, 30));
                }
                else if(timer_round_in < 0.1 * FPS)
                {
                    ...
                    round_splash[round].draw(WINDOW_WIDTH / 2 - round_splash[round].w * xscalar / 2 - roll(10, 30), WINDOW_HEIGHT / 2 - round_splash[round].h * scalar / 2 - roll(10, 30));
                }
                else round_splash[round].draw(WINDOW_WIDTH / 2 - round_splash[round].w / 2 - roll(5, 15), WINDOW_HEIGHT / 2 - round_splash[round].h / 2 - roll(5, 15));
            }
        }
        ...
        if(timer_ko)
        {
            if(timer_ko > 0.8 * FPS)
            {
                ...
                Graphics::setScale(scalar);
                ko[ko_type].draw(WINDOW_WIDTH / 2 - ko[ko_type].w * scalar / 2 - roll(10, 30), WINDOW_HEIGHT / 2 - ko[ko_type].h * scalar / 2 - roll(10, 30));
            }
            else if(timer_ko < 0.1 * FPS)
            {
                ...
                float xscalar = 1 / scalar;
                Graphics::setScale(xscalar, scalar);
                ko[ko_type].draw(WINDOW_WIDTH / 2 - ko[ko_type].w * xscalar / 2 - roll(10, 30), WINDOW_HEIGHT / 2 - ko[ko_type].h * scalar / 2 - roll(10, 30));
            }
            else ko[ko_type].draw(WINDOW_WIDTH / 2 - ko[ko_type].w / 2 - roll(5, 15), WINDOW_HEIGHT / 2 - ko[ko_type].h / 2 - roll(5, 15));
        }
    }
}
...
void MenuNetplay::think()
{
        ...
        switch(mode)
        {
            ...
            case MODE_SERVER:
            {
                ...
                else
                {
                    ...
                    if(input(INPUT_DIRMASK)) sndMenu.play();
                    if(input(INPUT_UP))
                    {
                        if(++portStr[digit-1] > 9) portStr[digit-1] = 0;
                    }
                    else if(input(INPUT_DOWN))
                    {
                        if(--portStr[digit-1] < 0) portStr[digit-1] = 9;
                    }
            ...
            case MODE_CLIENT:
            {
                    ...
                    if(input(INPUT_A))
                    {
                        ...
                        else if(choice == 3)
                        {
                            ...
                            for(int i = 0; ; i++)
                            {
                                if(!_sz_input[i])
                                {
                                    if(!_sz_ip) _success = false;
                                    else if(!_sz_port) _sz_port = _sz_input + i;
                                    break;
                                }
                                if(_sz_input[i] >= '0' && _sz_input[i] <= '9')
                                {
                                    if(!_sz_ip) _sz_ip = _sz_input + i;
                                }
                                else if(_sz_input[i] == ':')
                                {
                ...
                else
                {
                    if(input(INPUT_A))
                    {
                        //Save changes
                        sndSelect.play();
                        if(choice == 1) updateIp(true);
                        else if(choice == 2) updatePort(true);
                        digit = 0;
                    }
                    else if(input(INPUT_B))
                    {
                        //Discard changes
                        sndBack.play();
                        if(choice == 1) updateIp(false);
                        else if(choice == 2) updatePort(false);
                        digit = 0;
                    }
                    if(input(INPUT_DIRMASK)) sndMenu.play();
                    if(choice == 1)
                    {
                        if(input(INPUT_UP))
                        {
                            if(++ipStr[digit-1] > 9) ipStr[digit-1] = 0;
                        }
                        else if(input(INPUT_DOWN))
                        {
                            if(--ipStr[digit-1] < 0) ipStr[digit-1] = 9;
                        }
                        else if(input(INPUT_LEFT))
                        {
                            if(--digit < 1) digit = 12;
    ...
    barPos--;
    if(barPos < -NET_BAR_SIZE) barPos = WINDOW_HEIGHT + NET_BAR_SIZE;
}
...
void MenuNetplay::draw()
{
    ...
    if(flashTimer > NET_FLASH_TIME)
    {
        ...
        imgStatic.draw(-roll(WINDOW_WIDTH), -roll(WINDOW_HEIGHT));
...
void MenuCredits::parseLine(Parser& parser)
{
    ...
    else if(parser.is("CREDITS", 2))
    {
        ...
        sz_lines = getLinesFromFile(&c_lines, getResource(parser.getArg(2), EXT_TEXT));
    }
