...
void Projectile::handleFrame(ubyte_t command_)
{
    ...
    case STEP_Destroy:
        ...
        return;
    case STEP_Visible:
        if(readByte()) flags |= F_VISIBLE;
        else flags &= ~F_VISIBLE;
        break;
    case STEP_Invincible:
        if(readByte()) flags |= F_INVINCIBLE;
        else flags &= ~F_INVINCIBLE;
        break;
    case STEP_Gravity:
        if(readByte()) flags |= F_GRAVITY;
        else flags &= ~F_GRAVITY;
        break;
    case STEP_Airdash:
        if(readByte()) flags |= F_AIRDASH;
        else flags &= ~F_AIRDASH;
        if(flags & F_ON_GROUND) flags &= ~F_AIRDASH;
        break;
    case STEP_DoubleJump:
        if(readByte()) flags |= F_DOUBLEJUMP;
        else flags &= ~F_DOUBLEJUMP;
        if(flags & F_ON_GROUND) flags &= ~F_DOUBLEJUMP;
        break;
    case STEP_Mirror:
        if(readByte()) flags |= F_MIRROR;
        else flags &= ~F_MIRROR;
        break;
    case STEP_Bounce:
...
void Projectile::playSound(int id)
{
    ...
    if(id < 0 || id >= fighter->c_sounds)
        return;
    fighter->sounds[id].sounds[roll(fighter->sounds[id].size)].play();
}
...
void Projectile::say(int id)
{
    ...
    if(roll(100) < fighter->voices[id].pct)
    {
        p->speaker.play(&fighter->voices[id].voices[roll(fighter->voices[id].size)], fighter->voices[id].pct < 30);
    }
}
...
void Player::handleInput()
{
    ...
    if(dir == LEFT) input = flipInput(this->input);
    ...
    if(flags & F_CTRL)
    {
        ...
        if(!executed)
        {
            ...
            for(int i = 0; i < fighter->c_commands; i++)
            {
                ...
                {
                    uint16_t cmp = input;
                    if(!(fighter->commands[i].combo[0] & INPUT_DIRMASK)) cmp &= INPUT_KEYMASK;
                    if(keycmp(cmp, fighter->commands[i].combo[0], fighter->commands[i].generic & 1))
                    {
                        //Clear out the UP bit if the command used it
                        if(fighter->commands[i].combo[0] & INPUT_UP) this->input &= ~INPUT_UP;
                        if(executeCommand(i))
                        {
                            executed = true;
                            break;
...
void Player::think()
{
    ...
    if(!framePauseTimer)
    {
        ...
        else
        {
            ...
            if(!(flags & F_ON_GROUND))
            {
                ...
                float oldvel = vel.y;
                if(flags & F_GRAVITY) vel.y -= fighter->gravity;
                if(oldvel > 0 && vel.y < 0)
                {
                    if(!isAttacking() && !isBeingHit()) setStandardState(STATE_BEGIN_FALL);
                ...
                if(pos.y < 0)
                {
                    ...
                    else
                    {
                        ...
                        if(isKnockedBack())
                        {
                            ...
                            {
                                setStandardState(STATE_KB_BOUNCE);
                                if(vel.y == 0.0f) vel.y = BOUNCE_VELOCITY;
                                flags &= ~F_ON_GROUND;
                                flags |= F_BOUNCE;
                                juggle = JUGGLE_MIN;
                            }
    ...
    //Scale!
    if(idealScale != scale) scale = (scale + idealScale) / 2;
...
void Player::handleFrame(ubyte_t command_)
{
    ...
    case STEP_Cancel:
        ...
        if(c_cancel >= CANCEL_MAX) c_cancel = CANCEL_MAX - 1;
        break;
...
void Player::shootProjectile()
{
    ...
    if(fighter->sprites[sprite].aHitBoxes.size && shoot.state != STATE_NONE)
    {
        int mirror1 = 1;
        int mirror2 = 1;
        if(isMirrored()) mirror1 = -1;
        if(dir == LEFT) mirror2 = -1;
        ...
        if(++projectileId >= MAX_PROJECTILES) projectileId = 0;
...
void Projectile::interact(Projectile* other)
{
    ...
    Player* pother = NULL;
    Player* pself = NULL;
    if(other->isPlayer()) pother = (Player*)other;
    if(isPlayer()) pself = (Player*)this;
        ...
        if(!frameHit && attack.damage)
        {
            ...
            if(hit == HIT_HIT)
            {
                ...
                if(pother)
                {
                    ...
                    {
                        if(attack.type == HT_MID) blocked = true;
                        else if(pother->isJumping()) blocked = true;
                        else if(attack.type == HT_HIGH)
                        {
                            if(pother->isStanding()) blocked = true;
                        }
                        else if(attack.type == HT_LOW)
                        {
                            if(pother->isCrouching()) blocked = true;
                        }
                    }

                    if(blocked)
                    {
                        newSpark(colpos.x, colpos.y, 0, dir == LEFT);
                        ...
                        if(_force > 0 && (flags & F_ON_GROUND))
                        {
                            if(pother->pos.x + pother->fighter->widthLeft + STAGE_BUFFER >= STAGE.width || pother->pos.x - pother->fighter->widthLeft - STAGE_BUFFER <= -STAGE.width)
                                vel.x = _force * -mirror;
                            else pother->vel.x = _force * mirror;
                        }
                        else pother->vel.x = _force * mirror;
                    ...
                    {
                        if(pself) pself->comboCounter++;

                        newSpark(colpos.x, colpos.y, spark, dir == LEFT);
                        ...
                        if(knockdownOther) pother->flags |= F_KNOCKDOWN;
    ...
    if(pother && pself)
    {
        ...
        if(dir == RIGHT) me.pos.x = pos.x - fighter->widthLeft;
        else me.pos.x = pos.x - fighter->widthRight;

        HitBox you;
        you.size.x = other->fighter->widthLeft + other->fighter->widthRight;
        you.pos.y = other->pos.y;
        you.size.y = other->fighter->height;
        if(other->dir == RIGHT) you.pos.x = other->pos.x - other->fighter->widthLeft;
        else you.pos.x = other->pos.x - other->fighter->widthRight;
...
bool Player::isBlocking()
{
    ...
    if(flags & F_DEAD) return false;
    if(isAttacking() || (!(flags & F_CTRL) && !isInBlock())) return false;
    if(dir == LEFT && (input & INPUT_RIGHT)) return true;
    if(dir == RIGHT && (input & INPUT_LEFT)) return true;
    return false;
}
