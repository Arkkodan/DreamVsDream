...
const char* stateNames[STATE_MAX] =
{
    "Stand",
    "Crouch",
    "BeginStand",
...
#define STEP_END() }
        ...
        if(parser.isGroup())
        {
            ...
            for(int k = 0; k < argc; k++)
            {
                ...
                for(int i = 0; cmd[i]; i++)
                {
                    switch(cmd[i])
                    {
                    case '~': shift = INPUT_RELSHIFT; break;
                    case '1': key = (INPUT_LEFT | INPUT_DOWN) << shift; break;
                    case '2': key = INPUT_DOWN << shift; break;
                    case '3': key = (INPUT_RIGHT | INPUT_DOWN) << shift; break;
                    case '4': key = INPUT_LEFT << shift; break;
                    case '6': key = INPUT_RIGHT << shift; break;
                    case '7': key = (INPUT_LEFT | INPUT_UP) << shift; break;
                    case '8': key = INPUT_UP << shift; break;
                    case '9': key = (INPUT_RIGHT | INPUT_UP) << shift; break;
                    case 'd': key = INPUT_DOWN << shift; generic = true; break;
                    case 'u': key = INPUT_UP << shift; generic = true; break;
                    case 'b': key = INPUT_LEFT << shift; generic = true; break;
                    case 'f': key = INPUT_RIGHT << shift; generic = true; break;
                    case 'A': key |= INPUT_A << shift; break;
                    case 'B': key |= INPUT_B << shift; break;
                    case 'C': key |= INPUT_C << shift; break;
                    }
                }

                commands[commandCounter].combo[k] = key;
                if(generic) commands[commandCounter].generic |= (1<<k);
    ...
    for(int i = 0; i < STATE_MAX; i++)
    {
        ...
        if(found) statesStandard[i] = j;
        else statesStandard[i] = -1;
    }
