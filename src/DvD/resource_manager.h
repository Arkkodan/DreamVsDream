...
namespace resource_manager {
  ...
  /// @param reloadOK Reload the resource if true. Attempt to find resource if
  /// false.
  ...
  /// @typeparam T Any supported resource type (see implementation file)
  /// @param resource Name of a resource
  template <typename T> void unload(const std::string &resource);
  ...
  template <typename T> T *getResource(const std::string &resource);

  /// @brief An overrideable method to load resources from a manifest file
  /// @typeparam T Any supported resource type (see implementation file)
  template <typename T> void loadFromManifest();
  ...
  template <typename T> std::vector<T *> getFromManifest();
} // namespace resource_manager
