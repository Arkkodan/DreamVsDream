...
Animation::Animation()
    : nFrames(0), frames(), frameTimes(), playing(false), startFrame(0) {}

Animation::Animation(Animation &&other) noexcept
    : nFrames(other.nFrames), frames(), frameTimes(), playing(other.playing),
...
Animation::Animation(const std::string &filename)
  ...
  gif = stbi_load_gif_from_memory(
      ...
      static_cast<int>(bufferSize), &delays, &width, &height, &nFrames, &comp,
      STBI_default);
  if (!gif) {
    error = true;
    goto end;
  ...
  for (int i = 0; i < nFrames; i++) {
    // Create the Image
    frames[i].createFromMemory(&gif[i * stride], width, height,
                               Image::COLORTYPE_RGBA, nullptr);
