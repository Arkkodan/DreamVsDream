...
namespace resource_manager {
  ...
  static std::unordered_map<std::string,
                            std::unique_ptr<effect::EffectAnimation>>
      effectsMap;
} // namespace resource_manager

template <>
bool resource_manager::load<effect::EffectAnimation>(
    const std::string &resource, bool reloadOK) {
  if (!reloadOK) {
...
template <>
void resource_manager::unload<effect::EffectAnimation>(
    const std::string &resource) {
  effectsMap.erase(resource);
}

template <>
effect::EffectAnimation *resource_manager::getResource<effect::EffectAnimation>(
    const std::string &resource) {
  if (effectsMap.find(resource) == effectsMap.cend()) {
...
template <> void resource_manager::loadFromManifest<effect::EffectAnimation>() {
  ...
  for (const auto &j_effect : j_effects) {
    if (j_effect.is_string()) {
      if (std::find(effectsManifest.cbegin(), effectsManifest.cend(),
                    j_effect) == effectsManifest.cend()) {
...
template <>
std::vector<effect::EffectAnimation *>
resource_manager::getFromManifest<effect::EffectAnimation>() {
...
namespace resource_manager {
  ...
  static std::unordered_map<std::string, std::unique_ptr<game::Fighter>>
      fightersMap;
} // namespace resource_manager

template <>
bool resource_manager::load<game::Fighter>(const std::string &resource,
                                           bool reloadOK) {
  if (!reloadOK) {
...
template <>
game::Fighter *
resource_manager::getResource<game::Fighter>(const std::string &resource) {
  if (fightersMap.find(resource) == fightersMap.cend()) {
...
template <> void resource_manager::loadFromManifest<game::Fighter>() {
  ...
  for (const auto &j_fighter : j_fighters) {
    if (j_fighter.is_string()) {
      if (std::find(fightersManifest.cbegin(), fightersManifest.cend(),
                    j_fighter) == fightersManifest.cend()) {
...
template <>
std::vector<game::Fighter *>
resource_manager::getFromManifest<game::Fighter>() {
...
bool resource_manager::load<Font>(const std::string &resource, bool reloadOK) {
  ...
  std::string path =
      util::getPath("fonts/" + resource + '.' + Parser::EXT_FONT);
  if (!util::fileExists(path)) {
    return false;
  }
...
template <> void resource_manager::unload<Font>(const std::string &resource) {
...
namespace resource_manager {
  ...
  static std::unordered_map<std::string, std::unique_ptr<audio::Sound>>
      soundsMap;
} // namespace resource_manager

template <>
bool resource_manager::load<audio::Sound>(const std::string &resource,
                                          bool reloadOK) {
  if (!reloadOK) {
  ...
  std::string path =
      util::getPath("sounds/" + resource + '.' + Parser::EXT_SOUND);
  if (!util::fileExists(path)) {
    return false;
  }
...
template <>
audio::Sound *
resource_manager::getResource<audio::Sound>(const std::string &resource) {
  if (soundsMap.find(resource) == soundsMap.cend()) {
...
template <> void resource_manager::unload<Stage>(const std::string &resource) {
...
template <> void resource_manager::loadFromManifest<Stage>() {
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (j_stage.is_string()) {
      if (std::find(stagesManifest.cbegin(), stagesManifest.cend(), j_stage) ==
          stagesManifest.cend()) {
...
template <> std::vector<Stage *> resource_manager::getFromManifest<Stage>() {
