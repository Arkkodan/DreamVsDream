...
#define th_return \
  ...
  void run() {
    ...
    memset((char *)&temp, 0,
           sizeof(temp)); // For receiving data; don't read directly into "you"
    ...
    for (;;) {
      ...
      case NETSTATE_INIT: {
        if (mode == MODE_SERVER) {
          ...
          // Now wait for a message and perform a handshake which is totally not
          // a ripoff of TCP.
          ...
          if (recv(&buff, sizeof(buff))) {
            ...
            if (buff.flags & NetHeader::NETF_SYN &&
              ...
              if (send(&buff, sizeof(buff))) {
                ...
                if (recv(&buff, sizeof(buff))) {
                  // Check if its an ACK; if so, connection established; send
                  // calculated input delay
                  if (buff.flags & NetHeader::NETF_ACK &&
      ...
      case NETSTATE_LISTEN: {
        ...
        if (size + 1) {
          ...
          // Resend input information; this should be very rare, since we're
          // sending multiple inputs in case of packet loss often
          case PACKET_REQUEST: {
            if (size < 4) {
              break;
            }
  ...
  int send(const void *data, size_t size) {
    ...
    // Send packet
    if ((sent = sendto(sock, (const char *)data, size, 0, (sockaddr *)&you,
                       slen)) <= 0) {
      // free(buff);
      goto handleError;
    }
  ...
  int recv(void *data, size_t size) {
    ...
    for (;;) {
      ...
      // If we're getting data from the pipe, it means that we're being
      // terminated
...
#endif
        if ((sent = recvfrom(sock, (char *)data, size, 0, (sockaddr *)&temp,
                             &slen)) <= 0) {
          goto handleError;
        }
#ifndef _WIN32
