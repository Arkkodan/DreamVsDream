...
void usage() {
  fprintf(stderr, "fontbuild - build a font for use in Dream vs. "
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < dataSize; i++) {
    if (dataL[i] == '\n' || dataL[i] == '\f' || dataL[i] == '\r') {
      ...
      if (i && dataL[i - 1]) {
        ...
        else if (!strcmp(dataL + first, "var"))
          mono = 0;
        else if (!strcmp(dataL + first, "insensitive"))
          sensitive = 0;
        else if (!strcmp(dataL + first, "sensitive"))
          sensitive = 1;
        option++;
      }
    }
  ...
  for (; i < dataSize; i++) {
    ...
    if (flag) {
      ...
      char buff[8];
      int j;
      for (j = 0; dataL[i + j] && dataL[i + j] >= '0' && dataL[i + j] <= '9';
           j++)
        ;
