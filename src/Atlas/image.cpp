...
void imageRead(std::string szFileName_, struct Image *img_) {
  ...
  png_read_png(png_ptr, info_ptr,
               PNG_TRANSFORM_STRIP_16 | PNG_TRANSFORM_PACKING, nullptr);
  ...
  if (colortype != PNG_COLOR_TYPE_GRAY && colortype != PNG_COLOR_TYPE_PALETTE) {
    std::cerr << "error: file " << szFileName_
              << " is not a grayscale or indexed image." << std::endl;
    goto end;
  }
...
int imageReadPalette(std::string szFileName_, unsigned char *palette_) {
  ...
  if (colortype != PNG_COLOR_TYPE_PALETTE) {
    std::cerr << "error: file " << szFileName_
              << " uses an incorrect pixel type." << std::endl;
    code = 1;
    goto end;
  }
...
void imageWrite(std::string szFileName_, struct Image *img_,
  ...
  if (f == nullptr) {
    std::cerr << "error: cannot write to \"" << szFileName_ << "\"."
              << std::endl;
    goto end;
  }
  ...
  // Write header
  png_set_IHDR(png_ptr, info_ptr, img_->w, img_->h, 8,
               palette_ ? PNG_COLOR_TYPE_PALETTE : PNG_COLOR_TYPE_GRAY,
