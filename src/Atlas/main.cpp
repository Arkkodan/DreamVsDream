...
inline void imgcpy(struct Image *image, int x, int y) {
  for (int j = 0; j < image->h; j++) {
    for (int i = 0; i < image->w; i++) {
      atlas.data[TEXTURE_SIZE_SQ * i_atlas + TEXTURE_SIZE * (y + j) + (x + i)] =
          image->data[image->w * j + i];
    }
  }
}
...
void populate(int x, int y, int w, int h) {
  ...
  for (;;) {
    ...
    for (i = 1; i < c_fit;
         i++) { // Skip over first; it'll never have room under it
      populate(images[b_fit[i]].x, y + oy + images[b_fit[i]].h,
               images[b_fit[i]].w, images[b_fit[0]].h - images[b_fit[i]].h);
    }
...
int loadPalette(std::string szPalette, unsigned char *palette) {
  ...
  if (!ext.compare("act")) {
    ...
    std::cerr << "error: unrecognized palette file: " << ext << "."
              << std::endl;
    return 1;
  }
  return 0;
...
#endif
{
    ...
    if (sz_palette2.length()) {
      ...
      for (int i = 255; i >= 0; i--) {
        if (palette[i * 3 + 0] || palette[i * 3 + 1] || palette[i * 3 + 2]) {
          memcpy(palette + (i + 1) * 3, palette2 + (i + 1) * 3,
                 (256 - (i + 1)) * 3);
          break;
        }
      }
  ...
  for (;;) {
    ...
    // Write to file
    imageWrite(sz_output + "/" + util::toString(i++) + ".png", &atlas,
               sz_palette.empty() ? nullptr : palette);
