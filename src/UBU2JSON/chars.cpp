...
ubu::States::States(const std::string& file)
	: UBU(file)
{}
...
ubu::Commands::Commands(const std::string& file)
	: UBU(file)
{}
...
bool ubu::Commands::parse() {
	...
	while (parser.parseLine()) {
		...
		if (parser.isGroup()) {
			...
			targetIndex++;
			j_obj[commandIndex]["targets"][targetIndex]["state"] = parser.getArg(0).substr(1);
			conditionIndex = 0;
		}
		else {
			j_obj[commandIndex]["targets"][targetIndex]["conditions"][conditionIndex] = parser.getArg(0);
			conditionIndex++;
		}
	}
