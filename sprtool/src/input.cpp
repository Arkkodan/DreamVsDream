...
namespace Input
{
    ...
    HitBox* selectBox = NULL;
    bool selectBoxAttack = false;
    Vector selectBoxOffset;
    ...
    void keyPress(int key, bool press)
    {
        ...
        if(press)
            ...
            case SDLK_q:
                if(blackBG) Graphics::setClearColor(255, 255, 255);
                else Graphics::setClearColor(0, 0, 0);
                blackBG = !blackBG;
                break;
            ...
            case SDLK_c:
                ...
                {
                    copyFrame = frame;
                    if(selectAll) copyBoxes = true;
                    else copyBoxes = false;
                }
                break;
            ...
            case SDLK_v:
                if(ctrl)
                {
                    if(copyFrame == frame || copyFrame == -1) return;
            ...
            //Delete hitbox
            case SDLK_x:
                if(selectBoxAttack) fighter.sprites[frame].aHitBoxes.deleteHitbox(selectBox);
                else fighter.sprites[frame].hitBoxes.deleteHitbox(selectBox);
                selectBox = NULL;
                break;
                    ...
                    for(; i < fighter.sprites[frame].hitBoxes.size; i++)
                        if(fighter.sprites[frame].hitBoxes.boxes + i == selectBox)
                        {
                            if(i == 0)
                            {
                                ...
                                else selectBox = fighter.sprites[frame].hitBoxes.boxes + (fighter.sprites[frame].hitBoxes.size-1);
                            }
                            else selectBox = fighter.sprites[frame].hitBoxes.boxes + (i - 1);
                            break;
                        }
                        ...
                        for(i = 0; i < fighter.sprites[frame].aHitBoxes.size; i++)
                            if(fighter.sprites[frame].aHitBoxes.boxes + i == selectBox)
                            {
                                if(i == 0)
                                {
                                    ...
                                    else selectBox = fighter.sprites[frame].aHitBoxes.boxes + (fighter.sprites[frame].aHitBoxes.size-1);
                                }
                                else selectBox = fighter.sprites[frame].aHitBoxes.boxes + (i - 1);
                                break;
                            }
                    }
                }
                    ...
                    for(; i < fighter.sprites[frame].hitBoxes.size; i++)
                        if(fighter.sprites[frame].hitBoxes.boxes + i == selectBox)
                        {
                            if(i == fighter.sprites[frame].hitBoxes.size-1)
                            {
                                ...
                                else selectBox = fighter.sprites[frame].hitBoxes.boxes;
                            }
                            else selectBox = fighter.sprites[frame].hitBoxes.boxes + (i + 1);
                            break;
                        }
                        ...
                        for(i = 0; i < fighter.sprites[frame].aHitBoxes.size; i++)
                            if(fighter.sprites[frame].aHitBoxes.boxes + i == selectBox)
                            {
                                if(i == fighter.sprites[frame].aHitBoxes.size-1)
                                {
                                    ...
                                    else selectBox = fighter.sprites[frame].aHitBoxes.boxes;
                                }
                                else selectBox = fighter.sprites[frame].aHitBoxes.boxes + (i + 1);
                                break;
                            }
            ...
            case SDLK_LEFT:
                if(selectBox || selectAll)
                {
                    if(shift)
                    {
                        if(ctrl) selectBox->size.x -= 10;
                        else selectBox->size.x--;
                    ...
                    else
                    {
                        if(selectAll)
                        {
                            if(ctrl) moveAll(-10, 0);
                            else moveAll(-1, 0);
                        }
                        else
                        {
                            if(ctrl) selectBox->pos.x -= 10;
                            else selectBox->pos.x--;
                        }
                    }
                }
                else
                {
                    if(ctrl) fighter.sprites[frame].x += 10;
                    else fighter.sprites[frame].x++;
                }
                break;
            ...
            case SDLK_UP:
                if(selectBox || selectAll)
                {
                    if(shift)
                    {
                        if(ctrl) selectBox->size.y += 10;
                        else selectBox->size.y++;
                    }
                    else
                    {
                        if(selectAll)
                        {
                            if(ctrl) moveAll(0, 10);
                            else moveAll(0, 1);
                        }
                        else
                        {
                            if(ctrl) selectBox->pos.y += 10;
                            else selectBox->pos.y++;
                        }
                    }
                }
                else
                {
                    if(ctrl) fighter.sprites[frame].y -= 10;
                    else fighter.sprites[frame].y--;
                }
                break;
    ...
    void mouseMove(int x, int y)
    {
        ...
        float ratio = 1.0f;
        if(screenOffset.x) ratio = (float)WINDOW_HEIGHT / screenSize.y;
        else if(screenOffset.y) ratio = (float)WINDOW_WIDTH / screenSize.x;
        mousePos.x = (x - screenOffset.x) * ratio;
        mousePos.y = (y - screenOffset.y) * ratio;
        if(mousePos.x < 0) mousePos.x = 0;
        if(mousePos.x >= WINDOW_WIDTH) mousePos.x = WINDOW_WIDTH - 1;
        if(mousePos.y < 0) mousePos.y = 0;
        if(mousePos.y >= WINDOW_HEIGHT) mousePos.y = WINDOW_HEIGHT - 1;
